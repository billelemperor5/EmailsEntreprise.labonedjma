    ...
    try {
        ...
        if (doc.exists && doc.data().version) {
            ...
            if (cloudVersion !== CURRENT_VERSION) {
                ...
                // Self-healing: Automatically clean cache and reload for old users to get the new version.
                // We check sessionStorage to prevent any potential infinite reload loop.
                const lastAutoReload = sessionStorage.getItem('last-auto-reload');
                const now = Date.now();
                if (!lastAutoReload || (now - parseInt(lastAutoReload, 10)) > 60000) {
                    sessionStorage.setItem('last-auto-reload', String(now));
                    console.log('Detecting version mismatch. Clearing cache and reloading...');
