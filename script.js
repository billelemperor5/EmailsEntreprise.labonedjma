// ================================================
// LABO NEDJMA — EMAIL TRACKING SYSTEM v5.5.1
...
function toggleCorrectionUnitField() {
    ...
    const field = document.getElementById('corUnitField');
    if (field) field.style.display = (type === 'unit') ? 'block' : 'none';
}
...
document.getElementById('sigName')?.addEventListener('input', (e) => {
    const val = e.target.value.trim();
    if (!val) return;

    // Find a match in companyEmails
    const match = companyEmails.find(item => item.name && item.name.toLowerCase() === val.toLowerCase());
    if (match) {
        ...
        if (locationInput) {
            ...
            if (unit.includes('larb')) {
                locationInput.value = "Cité El Fahs Zone d’Activité Larbâa – Blida";
            } else if (unit.includes('douera')) {
                locationInput.value = "Lotissement 4 N°11 Douera, Alger - Algérie";
            } else if (unit.includes('smar')) {
                locationInput.value = "Zone industrielle Oued Semmar";
            } else {
                locationInput.value = ""; // Manual entry for others
            }
        }
...
function toggleCorrectionUnitField() {
    ...
    const field = document.getElementById('corUnitField');
    if (field) {
        field.style.display = type === 'unit' ? 'block' : 'none';
        const select = document.getElementById('corNewUnit');
        if (type !== 'unit' && select) select.value = '';
    }
}
    ...
    if (correctionForm) {
        correctionForm.addEventListener('submit', async (e) => {
            ...
            const correctionData = {
                ...
                correctionType: document.getElementById('corType').value,
                newUnit: document.getElementById('corType').value === 'unit' ? document.getElementById('corNewUnit').value : null,
...
async function approveRequest(requestId) {
    try {
        ...
        if (data.type === 'correction') {
            ...
            if (instrDesc) instrDesc.textContent = data.description;
