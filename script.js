...
// =============================================
// SPLASH SCREEN — Premium v3.0 (Cloud Sync Integrated) — Mobile Optimized
// =============================================
async function initSplashScreen() {
    ...
    // Optimize: Reduce animation frequency on mobile to prevent jank
    const progressUpdateInterval = isMobile ? 100 : 50;
    const progressStep = isMobile ? 4 : 1;
    ...
    function updateProgressSmooth() {
        if (pct < (perfLite ? 80 : 90)) {
            ...
            progressInterval = setTimeout(updateProgressSmooth, progressUpdateInterval);
        }
    }
...
function navigateTo(page, options = {}) {
    ...
    if (window.matchMedia('(max-width: 768px)').matches) {
        window.scrollTo(0, 0); // Instant scroll on mobile
    } else {
...
function animateValue(el, start, end, duration) {
    ...
    // On mobile, skip animation or reduce duration
    const isMobile = window.matchMedia('(max-width: 768px)').matches;
    if (isMobile) {
        el.innerText = end;
        return; // Just set the value directly on mobile
    }
...
function goToPage(page) {
    ...
    if (page >= 1 && page <= totalPages) {
        ...
        // Use instant scroll on mobile, smooth scroll on desktop
        const isMobile = window.matchMedia('(max-width: 768px)').matches;
...
function updateDirectoryStats(emails) {
    ...
    const updates = {
        totalCountEl: total,
        activeCountEl: active,
        blockedCountEl: blocked,
        activationRateEl: `${percentage}%`
    };
    
    // Apply all updates at once
    if (totalCountEl) totalCountEl.innerText = updates.totalCountEl;
    if (activeCountEl) activeCountEl.innerText = updates.activeCountEl;
    if (blockedCountEl) blockedCountEl.innerText = updates.blockedCountEl;
    if (activationRateEl) activationRateEl.innerText = updates.activationRateEl;
    
    // Use transform instead of width for better GPU acceleration
    if (progressBar) {
...
const searchDebounceDelay = isMobileDevice ? 200 : 120; // Longer delay on mobile
...
searchInput.addEventListener('input', (e) => {
    ...
    searchDebounceTimer = setTimeout(() => {
        // Batch filtering and DOM updates
        const filtered = companyEmails.filter(item =>
