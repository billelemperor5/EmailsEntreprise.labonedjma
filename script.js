...
const IS_DEV_MODE = location.hostname === 'localhost' || location.hostname === '127.0.0.1' || location.protocol === 'file:';
...
function openModal(modalId) {
    devLog("Opening modal:", modalId);
    const targetId = modalId === 'editModal' ? 'emailAdminModal' : modalId;
    
    // Check if it's already open to prevent opening twice
...
function closeModal(modalId) {
    devDebug('[LOW LEVEL CLOSE]', modalId);
    if (modalId === 'editModal' || modalId === 'emailAdminModal') {
        devDebug('CLOSE EDIT');
    }
    const targetId = modalId === 'editModal' ? 'emailAdminModal' : modalId;
    const modal = document.getElementById(targetId);
