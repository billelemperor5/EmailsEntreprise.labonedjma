...
window.addEventListener("scroll", function() {
    ...
    if (currentScroll > lastScrollTop && currentScroll > 40) {
        // Scrolling Down: Hide Blue, Move White to Top
        if (govBanner) govBanner.classList.add("hide");
        if (mainNavbar) mainNavbar.style.top = "0";
    } else if (currentScroll < lastScrollTop) {
        // Scrolling Up: Show Blue, Move White Down
        if (govBanner) govBanner.classList.remove("hide");
        if (mainNavbar) mainNavbar.style.top = "var(--gov-bar-height)";
    }
