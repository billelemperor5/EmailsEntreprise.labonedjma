// ================================================
// LABO NEDJMA — EMAIL TRACKING SYSTEM v6.1.0
// Gov Clean Style — Navbar Edition
// ================================================

const DEBUG = false;

// Silence console in production environment if DEBUG is false
...
// IMPORTANT:
// Do NOT replace console.* inside these wrapper functions.
// They must always call the native browser console APIs.
// =======================================================================================
// SYSTEM VERSION MANAGER & AUTOMATIC UPGRADE PIPELINE
// =======================================================================================
const APP_VERSION = "6.1.0";
const CURRENT_VERSION = APP_VERSION; // Maintain compatibility

(function () {
    const storedVersion = localStorage.getItem('app_version');

    if (storedVersion !== APP_VERSION) {
        console.log(`[VersionManager] Upgrade detected from ${storedVersion || 'none'} to ${APP_VERSION}.`);

        const runMigration = async () => {
            // Show update screen with version comparison
            showMigrationOverlay(storedVersion, APP_VERSION);

            // Perform migration asynchronously to allow UI to render first
            setTimeout(async () => {
                const previousVersion = storedVersion || "0.0.0";

                try {
                    // Switch for specific migration steps
                    switch (previousVersion) {
                        case "6.0.4":
                            console.log("[Migration] Running migration steps for v6.0.4");
                            break;
                        default:
                            console.log("[Migration] Running default full cleanup migration");
                            break;
                    }

                    const cleanupPromises = [];

                    // 1. Clean Cache Storage (Service Worker Caches)
                    if ('caches' in window) {
                        cleanupPromises.push((async () => {
                            try {
                                const cacheNames = await caches.keys();
                                await Promise.all(cacheNames.map(name => caches.delete(name)));
                                console.log("[Migration] Cache cleared successfully");
                            } catch (err) {
                                console.warn("[Migration] Cache deletion failed:", err);
                            }
                        })());
                    }
                    ...
                    if ('serviceWorker' in navigator) {
                        cleanupPromises.push((async () => {
                            try {
                                ...
                                console.log("[Migration] Service Workers unregistered");
                            } catch (err) {
                                console.warn("[Migration] SW unregistration failed:", err);
                            }
                        })());
                    }

                    // 3. Clear IndexedDB local database
                    cleanupPromises.push((async () => {
                        try {
                            await new Promise((resolve) => {
                                const req = indexedDB.deleteDatabase('labo_nedjma_local_database');
                                req.onsuccess = () => {
                                    console.log("[Migration] IndexedDB database deleted");
                                    resolve();
                                };
                                req.onerror = () => {
                                    console.warn("[Migration] IndexedDB deletion failed (request error)");
                                    resolve();
                                };
                                req.onblocked = () => {
                                    console.warn("[Migration] IndexedDB delete blocked");
                                    resolve();
                                };
                            });
                        } catch (err) {
                            console.warn("[Migration] IndexedDB deletion failed:", err);
                        }
                    })());

                    // Wait for all async cleanups to finish completely before reload
                    await Promise.all(cleanupPromises);

                    // Clear App-specific LocalStorage cache keys (leaving settings/auth intact)
                    const keysToKeep = ['theme', 'rememberMe', 'app_version', 'previous_version', 'upgraded_at'];
                    ...
                    // Save new version and statistics
                    localStorage.setItem('previous_version', previousVersion);
                    localStorage.setItem('upgraded_at', new Date().toISOString());
                    localStorage.setItem('app_version', APP_VERSION);

                    console.log(`[VersionManager] Upgrade to ${APP_VERSION} completed successfully.`);
                    performReload();
...
function showMigrationOverlay(oldVersion, newVersion) {
    const displayOld = oldVersion ? oldVersion : "Aucune";
    const displayNew = newVersion || APP_VERSION;

    const style = document.createElement('style');
    style.id = 'migrationOverlayStyle';
    style.innerHTML = `
        .migration-overlay {
            position: fixed;
            inset: 0;
    ...
    overlay.innerHTML = `
        <div class="migration-card">
            ...
            <div class="migration-title">Mise à jour du système</div>
            <div class="migration-text">
                Une nouvelle version du système a été détectée.<br>
                <div class="migration-info-box">
                    <strong>Version actuelle :</strong> ${displayOld}<br>
                    <strong>Nouvelle version :</strong> ${displayNew}
                </div>
...
// =======================================================================================
// PERFORMANCE MONITOR & ADAPTIVE MOTION ENGINE v6.1.0
// =======================================================================================
const perfStats = {
    totalTransitions: 0,
    ...
    longestAnimationDuration: 0,
    bypassedTransitionsCount: 0,
    renderCount: 0,
    lcp: null,
    tti: null
};

// Setup PerformanceObservers
if (typeof PerformanceObserver !== 'undefined') {
    try {
        const lcpObserver = new PerformanceObserver((entryList) => {
            const entries = entryList.getEntries();
            if (entries.length > 0) {
                perfStats.lcp = entries[entries.length - 1].startTime;
            }
        });
...
window.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        if (typeof performance !== 'undefined' && performance.now) {
            perfStats.tti = performance.now();
        }
    }, 1000);
});

function cleanupTransitionResources() {
    const pageContent = document.getElementById('pageContent');
    if (pageContent) {
        pageContent.style.willChange = '';
    }
    document.querySelectorAll('.modal, .modal-overlay, .config-modal-overlay, .employee-details-overlay').forEach(modal => {
        const modalBox = modal.querySelector('.modal-content, .modal-box, .employee-details-modal, .success-modal-content, .request-modal-content') || modal;
        if (modalBox) {
    ...
    document.querySelectorAll('.stat-card, .module-card, .tool-card, .unit-stat-card, .db-card, .panel-card, .table-card, .service-card').forEach(card => {
        card.style.willChange = '';
        card.style.transform = '';
    });
}

async function measureTransition(name, callback) {
    const isReduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    const isMotionLite = document.documentElement.classList.contains('motion-lite');
    const isDev = DEBUG && (location.hostname === 'localhost' || location.hostname === '127.0.0.1' || location.protocol === 'file:');

    if (isReduced || isMotionLite) {
        perfStats.bypassedTransitionsCount++;
    }
    ...
    try {
        ...
        let averageFps = 60;
        let avgFrameDuration = 16.67;
        if (isDev && frameTimes.length > 1) {
            const intervals = [];
            for (let i = 1; i < frameTimes.length; i++) {
                intervals.push(frameTimes[i] - frameTimes[i - 1]);
            }
            avgFrameDuration = intervals.reduce((a, b) => a + b, 0) / intervals.length;
            averageFps = 1000 / avgFrameDuration;
        }

        perfStats.totalTransitions++;
        perfStats.totalTransitionTime += duration;
        perfStats.totalFPS += averageFps;
        if (duration > perfStats.longestAnimationDuration) {
            perfStats.longestAnimationName = name;
            perfStats.longestAnimationDuration = duration;
        }

        if (isDev) {
            const frameBudgetExceeded = avgFrameDuration > 16.7; // >16.7ms frame duration (FPS < 60)
            const totalDurationExceeded = duration > 300;

            if (frameBudgetExceeded || totalDurationExceeded) {
                console.warn(
                    `[Perf Warning] Transition "${name}" exceeded budget!\n` +
                    `  Duration: ${duration.toFixed(2)}ms (Limit: 300ms) ${totalDurationExceeded ? '❌' : '✅'}\n` +
                    `  Avg Frame Time: ${avgFrameDuration.toFixed(2)}ms (Limit: 16.7ms) ${frameBudgetExceeded ? '❌' : '✅'}\n` +
                    `  Avg FPS: ${averageFps.toFixed(1)} FPS`
                );
            } else {
                console.log(`[Perf Success] Transition "${name}" - Duration: ${duration.toFixed(2)}ms, FPS: ${averageFps.toFixed(1)}`);
            }
        }

        // Auto-degradation trigger (FPS < 35)
        if (averageFps < 35 && !isReduced && !isMotionLite && perfStats.totalTransitions >= 2) {
            if (isDev) {
                console.warn("[Perf Monitor] Performance bottleneck detected. Activating Motion Lite auto-degradation.");
            }
...
function getPerformanceReport() {
    const avgTime = perfStats.totalTransitions > 0 ? (perfStats.totalTransitionTime / perfStats.totalTransitions) : 0;
    const avgFps = perfStats.totalTransitions > 0 ? (perfStats.totalFPS / perfStats.totalTransitions) : 0;
    const isDev = DEBUG && (location.hostname === 'localhost' || location.hostname === '127.0.0.1' || location.protocol === 'file:');

    const report = {
        averageTransitionTimeMs: avgTime.toFixed(2),
        averageFps: avgFps.toFixed(1),
        longestAnimation: perfStats.longestAnimationName ? `${perfStats.longestAnimationName} (${perfStats.longestAnimationDuration.toFixed(2)}ms)` : 'N/A',
        totalTransitionsExecuted: perfStats.totalTransitions,
        totalTransitionsBypassed: perfStats.bypassedTransitionsCount,
        totalRenders: perfStats.renderCount,
        lcpMs: perfStats.lcp ? perfStats.lcp.toFixed(2) : 'N/A',
        ttiMs: perfStats.tti ? perfStats.tti.toFixed(2) : 'N/A'
    };

    if (isDev) {
        console.group("=== LABO NEDJMA PERFORMANCE REPORT ===");
...
window.measureTransition = measureTransition;
window.getPerformanceReport = getPerformanceReport;

if (DEBUG && (location.hostname === 'localhost' || location.hostname === '127.0.0.1' || location.protocol === 'file:')) {
    window.addEventListener('load', () => {
        setTimeout(() => {
            console.log("\n=== AUTOMATIC PERFORMANCE REPORT AFTER INITIAL BOOT ===");
            getPerformanceReport();
        }, 5000);
    });
}

const IS_DEV_MODE = DEBUG && (location.hostname === 'localhost' || location.hostname === '127.0.0.1' || location.protocol === 'file:');
...
if ('serviceWorker' in navigator &&
    ...
    window.addEventListener('load', () => {
        try {
            ...
            console.warn('Service worker registration error:', registerErr);
        }
    });
    ...
    function updateVisibilityState() {
        ...
        if (!document.hidden && window.__laboRenderPending) {
            ...
            if (typeof refreshApplicationState === 'function') {
                requestAnimationFrame(() => refreshApplicationState({ force: true, persist: false }));
            }
        }
...
function setupAuthStateListener() {
    ...
    auth.onAuthStateChanged((user) => {
        ...
        if (user) {
            ...
            devLog("Admin Logged In. Cloud Control Enabled.");

            // Setup realtime listeners for admin now that we are logged in
            ...
            // Clean up admin listeners to prevent leaks and console warnings
...
function initializeAppCore() {
    ...
    initModalPerformanceObserver(); // Initialize dynamic modal GPU acceleration observers
...
function isBlockedEmailStatus(status) {
    ...
    return s.startsWith('bloqu') || s.startsWith('desactiv') || s.startsWith('désactiv');
}
...
function applyIndexedDBPayload(payload) {
    if (currentOpenModal === 'editModal') {
        devLog('Sync/Refresh detected while edit modal is open. Closing edit modal safely.');
...
function getFilteredEmailsForActiveView() {
    ...
    if (activeView.type === 'unit') {
        ...
        if (activeView.value === 'Bloquée') {
            filtered = companyEmails.filter(e => isBlockedEmailStatus(e.status));
        } else {
...
let emailsListenerUnsubscribe = null;
let pendingRequestsListenerUnsubscribe = null;

function setupFirestoreRealtimeSync() {
    if (!navigator.onLine || !initializeFirebaseServices()) return;

    if (!emailsListenerUnsubscribe) {
        emailsListenerUnsubscribe = db.collection('emails').onSnapshot((snapshot) => {
            latestEmailsSnapshot = snapshot;
            scheduleFirestoreCacheSync();
        }, (err) => console.warn("Emails listener error:", err));
    }

    // Public realtime listener for pending requests to sync counts and stats for everyone
    if (!pendingRequestsListenerUnsubscribe) {
        pendingRequestsListenerUnsubscribe = db.collection('pendingRequests')
...
function cleanupPendingRequestsListener() {
    if (pendingRequestsListenerUnsubscribe) {
        pendingRequestsListenerUnsubscribe();
        pendingRequestsListenerUnsubscribe = null;
    }
    latestPendingRequestsSnapshot = null;
}

function cleanupAdminListeners() {
    if (adminNotifUnsubscribe) {
        adminNotifUnsubscribe();
        adminNotifUnsubscribe = null;
    }
    adminRequestNotifs = [];
}
...
async function fetchEmailsFromFirestore() {
    ...
    try {
        ...
        applyIndexedDBPayload(payload);
        setOfflineMode(false);
        devLog(`Successfully synced ${companyEmails.length} items from cloud.`);
        updateDbSyncStatus('synced');
        return true;
    } catch (error) {
...
const closeProfileModalBtn = document.getElementById('closeModal');
...
function navigateTo(page, options = {}) {
    ...
    const action = () => {
        ...
        // Toggle will-change for performance during transition
        const pageContent = document.getElementById('pageContent');
        if (pageContent) {
            pageContent.style.willChange = 'transform, opacity';
        }

        const cleanUpWillChange = () => {
            setTimeout(() => {
                if (pageContent) pageContent.style.willChange = '';
            }, 350);
        };

        if (document.startViewTransition && !window.matchMedia('(prefers-reduced-motion: reduce)').matches && !document.documentElement.classList.contains('performance-lite')) {
            const transition = document.startViewTransition(updateDOM);
            if (transition && typeof transition.finished !== 'undefined') {
                transition.finished.then(cleanUpWillChange).catch(cleanUpWillChange);
            } else {
                cleanUpWillChange();
            }
        } else {
    ...
    measureTransition('Navigation: ' + page, async () => {
        action();
...
function closeCompanyModal() {
    devDebug('[WRAPPER CLOSE] companyModal');
    closeModal('companyModal');
}

function closeDevModal() {
    devDebug('[WRAPPER CLOSE] devModal');
    closeModal('devModal');
}
...
function updateDashboardStats() {
    ...
    const blocked = companyEmails.filter(e => isBlockedEmailStatus(e.status) && !e.isRequest).length;
...
function renderEmails(emails, page = 1) {
    ...
    if (paginatedEmails.length === 0) {
        emailList.innerHTML = `<tr><td colspan="8" style="text-align: center; padding: 20px;">Aucun email trouvé.</td></tr>`;
    ...
    paginatedEmails.forEach((item, i) => {
        ...
        if (item.status === 'Désactivée') statusClass = 'status-deactivated';
        ...
        const isSelected = selectedEmails.has(item.mail);
        const isSelectable = isEmailSelectable(item.status);
        const rowSelectedClass = isSelected ? 'selected-row' : '';

        rowsHtml += `
            <tr class="email-mobile-card employee-row ${rowSelectedClass}" onclick="openEmployeeDetails('${encodeURIComponent(item.mail)}')" title="Voir la fiche professionnelle">
                <td style="text-align:center;" onclick="event.stopPropagation();" data-label="Sélection">
                    <input type="checkbox" class="email-select-checkbox" data-email="${item.mail}" ${isSelectable ? '' : 'disabled'} ${isSelected ? 'checked' : ''} aria-label="Sélectionner ${item.name}">
                </td>
...
function updateDirectoryStats(emails) {
    ...
    const blocked = list.filter(e => isBlockedEmailStatus(e.status)).length;
...
// =============================================
// =============================================
// EMAIL SELECTION & BATCH COPY SYSTEM (v6.1.0)
// =============================================

function isEmailSelectable(status) {
...
function cleanupSelection() {
    ...
    for (const email of selectedEmails) {
        ...
        if (!item || !isEmailSelectable(item.status)) {
...
function updateSelectAllHeaderCheckbox() {
    const selectAllCheckbox = document.getElementById('selectAllPageEmails');
    if (!selectAllCheckbox) return;

    const pageCheckboxes = Array.from(document.querySelectorAll('.email-select-checkbox:not(:disabled)'));
...
let isCopySelectionCooldown = false;
...
function updateCopySelectionBtn() {
    ...
    if (count > 0) {
        copySelectionBtn.innerHTML = `📧 Copier la sélection (${count})`;
        if (!isCopySelectionCooldown) {
            copySelectionBtn.disabled = false;
        }
    } else {
        copySelectionBtn.innerHTML = `📧 Copier la sélection`;
...
if (emailList) {
    emailList.addEventListener('change', (e) => {
        if (e.target.classList.contains('email-select-checkbox')) {
...
// Select-all checkbox listener (using change event delegation)
document.addEventListener('change', (e) => {
    if (e.target.id === 'selectAllPageEmails') {
        const isChecked = e.target.checked;
        const pageCheckboxes = document.querySelectorAll('.email-select-checkbox:not(:disabled)');
...
// Copy selection click handler
if (copySelectionBtn) {
    copySelectionBtn.addEventListener('click', () => {
        if (selectedEmails.size === 0 || isCopySelectionCooldown) return;

        // Smart Copy: preserve selection order (Set insertion order), deduplicate, validate format
        const cleanEmails = [];
        const seen = new Set();
        ...
        for (const email of selectedEmails) {
            ...
            if (trimmed && emailRegex.test(trimmed) && !seen.has(trimmed)) {
        ...
        const emailsText = cleanEmails.map(email => `${email};`).join('\n');

        copyTextToClipboard(emailsText, {
            count: cleanEmails.length,
            successMessage: `${cleanEmails.length} adresses copiées avec succès.`,
            showToast: false
        }).then((success) => {
            if (success) {
                ...
                isCopySelectionCooldown = true;
                copySelectionBtn.disabled = true;
                setTimeout(() => {
                    isCopySelectionCooldown = false;
                    updateCopySelectionBtn();
                }, 500);
            }
        });
    ...
    copySelectionBtn.addEventListener('keydown', (e) => {
        if (e.key === 'Enter' || e.key === ' ') {
            e.preventDefault();
            copySelectionBtn.click();
        }
    });
}
...
function renderUnitStats() {
    ...
    systemUnits.forEach((unitObj, idx) => {
        ...
        const blocked = unitEmails.filter(e => isBlockedEmailStatus(e.status)).length;
...
function closeUnitManagement() {
    closeModal('unitManagementModal');
}
...
async function deleteUnit(id) {
    unitIdToDelete = id;
    openModal('unitDeleteConfirmModal');
}

function closeUnitDeleteModal() {
    unitIdToDelete = null;
    closeModal('unitDeleteConfirmModal');
}
...
async function renderArchiveLog() {
    ...
    // Show skeleton if data retrieval takes more than 150ms
    const skeletonTimer = setTimeout(() => {
        if (!isDataLoaded) {
            archiveLog.innerHTML = Array(6).fill(0).map(() => `
                <div class="archive-item skeleton-item">
                    <span class="skeleton-text skeleton-shimmer" style="width: 70%; height: 12px; border-radius: 4px; display: inline-block;"></span>
                    <span class="skeleton-date skeleton-shimmer" style="width: 80px; height: 12px; border-radius: 4px; display: inline-block;"></span>
                </div>
            `).join('');
        }
    }, 150);
    ...
    const renderFallback = () => {
        ...
        const fallbackLogs = [
            ...
            { text: "Mise à jour de l'application vers la version v6.1.0", offset: 2 },
...
function closeProfileModal() {
    devDebug('[WRAPPER CLOSE] profileModal');
    closeModal('profileModal');
}

document.addEventListener('DOMContentLoaded', () => {
    if (infoBtn && profileModal && closeProfileModalBtn) {
...
function openToolDetail(toolName) {
    ...
    if (toolName === 'thunderbird') {
        openModal('thunderbirdModal');
        return;
    }
    if (toolName === 'webmail') {
        openModal('webmailModal');
        return;
    }
}
...
function closeConfigModal() {
    devDebug('[WRAPPER CLOSE] configModal');
    closeModal('configModal');
}

// Close install modal
function closeInstallModal() {
    devDebug('[WRAPPER CLOSE] installModal');
    closeModal('installModal');
}

// Close desktop modal
function closeDesktopModal() {
    devDebug('[WRAPPER CLOSE] desktopModal');
    closeModal('desktopModal');
}

// Close support modal
function closeSupportModal() {
    devDebug('[WRAPPER CLOSE] supportModal');
    closeModal('supportModal');
}

// Close thunderbird modal
function closeThunderbirdModal() {
    devDebug('[WRAPPER CLOSE] thunderbirdModal');
    closeModal('thunderbirdModal');
}

// Close webmail modal
function closeWebmailModal() {
    devDebug('[WRAPPER CLOSE] webmailModal');
    closeModal('webmailModal');
}
    ...
    form.addEventListener('submit', async function (e) {
        ...
        emailClient.send('service_vjznpgi', 'template_648u6p9', templateParams)
            .then(function (response) {
                devLog('SUCCESS!', response.status, response.text);
...
async function executeExcelExport(btn, originalText) {
    ...
    try {
        ...
        const headers = ['N°', 'Unité', 'Nom et Prénom', 'Poste', 'Adresse Email', 'Téléphone', 'Statut'];
        ...
        companyEmails.forEach((emp, idx) => {
            ...
            row.eachCell((cell, colNum) => {
                ...
                cell.alignment = { vertical: 'middle', horizontal: (colNum === 1 || colNum === 6 || colNum === 7) ? 'center' : 'left' };
                ...
                if (colNum === 7) {
                    ...
                    } else if (emp.status === 'Désactivée') {
                        cell.font = { bold: true, color: { argb: 'FF64748B' } };
                        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF1F5F9' } };
                    } else if (emp.status === 'En attente') {
...
// =============================================
// CURRENT_VERSION is now defined at the top under Version Manager
    ...
    try {
        ...
        if (typeof window.clearIndexedDB === 'function') {
            try {
                ...
                console.warn("clearIndexedDB() failed:", dbErr);
            }
        }

        // Check if Service Worker/Cache APIs are supported on the current origin
        const isSupportedOrigin = (location.protocol === 'https:' ||
        ...
        if (isSupportedOrigin) {
            // 4. Clear all Caches (Service Worker Caches)
            try {
                if ('caches' in window) {
                    ...
                    if (keys && keys.length > 0) {
                        await Promise.all(keys.map(key => {
                            try {
                                ...
                                console.warn(`Cache delete failed for key ${key}:`, cacheDelErr);
                ...
                console.warn("Caches deletion failed:", cacheErr);
            }
            ...
            try {
                if ('serviceWorker' in navigator) {
                    ...
                    if (registrations && registrations.length > 0) {
                        await Promise.all(registrations.map(r => {
                            try {
                                return r.unregister().catch(unregPromiseErr => {
                                    console.warn("ServiceWorker unregister promise rejection:", unregPromiseErr);
                                });
                            } catch (unregErr) {
                                console.warn("ServiceWorker unregister call failed:", unregErr);
                ...
                console.warn("ServiceWorker unregistration failed:", swErr);
            }
        } else {
            devLog("Service Worker / Cache APIs skipped due to unsupported protocol/origin:", location.protocol);
        }

        // 6. Show Success Modal
        openModal('cacheSuccessModal');

        try {
            addSystemLog(`Nettoyage complet du cache (Force update v${CURRENT_VERSION})`);
        } catch (logErr) {
            console.warn("addSystemLog failed:", logErr);
        }
...
function closeLogoutModal() {
    devDebug('[WRAPPER CLOSE] logoutConfirmModal');
...
async function migrateEmailsToFirestore() {
    ...
    try {
        ...
        devLog("Migration Successful. You can now switch to dynamic fetching.");
    } catch (error) {
...
function closeAuthModal() {
    devDebug('[WRAPPER CLOSE] authModal');
    closeModal('authModal');
}
...
// =======================================================================================
// IMPORTANT: GHOST CLICK INTERCEPTOR / SUPPRESSEUR DE GHOST CLICK
// =======================================================================================
// EXPLANATION / RATIONALE:
// When a modal overlay (like emailAdminModal) is closed, it transitions to hidden.
// On touch devices or hybrid screens, the browser generates mouse events (mousedown,
// mouseup, click) approximately 300ms after the touch events.
// Since the modal is closed synchronously, these delayed click events fall through the
// hidden overlay and trigger unwanted clicks on the main page elements underneath
// (such as table rows or buttons), resulting in details/edit modals reopening.
// 
// SOLUTION:
// We intercept and swallow the mousedown/mouseup/click sequence once only if it
// occurs at the exact same screen coordinates within 400ms of any modal closure.
// If the user clicked elsewhere, we clear the flags to not interfere with normal usage.
// DO NOT DELETE THIS SYSTEM. IT IS ESSENTIAL FOR MOBILE/TOUCH TAP-THROUGH STABILITY.
// =======================================================================================

let lastInteractionX = -1;
let lastInteractionY = -1;
let lastInteractionTime = 0;

let shouldIgnoreNextMousedown = false;
let shouldIgnoreNextMouseup = false;
let shouldIgnoreNextClick = false;

function updateLastInteraction(e) {
    // If we are currently waiting to swallow a ghost click, do NOT overwrite the target coordinates.
    // We want to keep the coordinates of the interaction that triggered the modal close.
    if (shouldIgnoreNextMousedown || shouldIgnoreNextMouseup || shouldIgnoreNextClick) {
        return;
    }
    let x = -1, y = -1;
...
window.addEventListener('mousedown', updateLastInteraction, true);
window.addEventListener('touchstart', updateLastInteraction, true);
window.addEventListener('pointerdown', updateLastInteraction, true);

function clearIgnoreFlags() {
    shouldIgnoreNextMousedown = false;
    shouldIgnoreNextMouseup = false;
    shouldIgnoreNextClick = false;
}

window.addEventListener('mousedown', (e) => {
    if (shouldIgnoreNextMousedown) {
        if (Date.now() - lastModalCloseTime < 400) {
            const dist = Math.hypot(e.clientX - lastInteractionX, e.clientY - lastInteractionY);
            if (dist < 20 || (e.clientX === 0 && e.clientY === 0)) {
                shouldIgnoreNextMousedown = false;
                e.preventDefault();
                e.stopPropagation();
...
function shouldBlockDeleteModal() {
    return isSavingEmail || isAdminModalOpen();
}
...
function syncDirectoryInBackground(viewState = null) {
    ...
    withOperationTimeout(syncAndRefreshFromFirestore({ render: true }), 'Directory sync', 8000)
        .then(() => {
            updateDbSyncStatus('synced');
...
// Wrapper Compatibility Functions
function openEditModal() {
    devDebug('openEditModal called');
    devDebug('CURRENT MODAL:', currentOpenModal);
    if (currentOpenModal === 'editModal') {
        console.warn('Edit modal already open');
        return;
    }
    openModal('emailAdminModal');
}

function editEmail(email, options = {}) {
    devDebug('editEmail called for:', email);
    devDebug('CURRENT MODAL:', currentOpenModal);
    if (currentOpenModal === 'editModal') {
        console.warn('Edit modal already open');
        return;
    }
    handleEditEmail(email, options);
}
...
async function handleEditEmail(email, options = {}) {
    devDebug('OPEN EDIT:', email);
    devDebug('CURRENT MODAL:', currentOpenModal);

    // If edit modal is already open, safely close it first before reloading
    if (currentOpenModal === 'editModal') {
        console.warn('Edit modal already open. Reinitializing for new email:', email);
        closeModal('editModal');
    }
...
function openEmployeeDetails(encodedEmail) {
    ...
    } else if (employee.status === 'Désactivée') {
        statusClass = 'is-deactivated';
    } else if (employee.status === 'Bloquée') {
...
function closeEmployeeDetailsModal() {
    devDebug('[WRAPPER CLOSE] employeeDetailsModal');
...
function editEmployeeFromDetails() {
    ...
    const email = currentEmployeeDetails.mail;
    closeEmployeeDetailsModal();
    handleEditEmail(email, { force: true });
}
...
let isRefreshStateInProgress = false;
let hasPendingRefreshState = false;
let pendingRefreshOptions = {};
let refreshDebounceTimer = null;
let stateVersion = 0;

async function refreshApplicationState(options = {}) {
    if (Object.keys(pendingRefreshOptions).length === 0) {
        pendingRefreshOptions = { ...options };
    } else {
        pendingRefreshOptions = {
            persist: pendingRefreshOptions.persist !== false || options.persist !== false,
            refreshPending: pendingRefreshOptions.refreshPending !== false || options.refreshPending !== false,
            refreshDirectory: pendingRefreshOptions.refreshDirectory !== false || options.refreshDirectory !== false,
            refreshDashboard: pendingRefreshOptions.refreshDashboard !== false || options.refreshDashboard !== false,
            refreshRecent: pendingRefreshOptions.refreshRecent !== false || options.refreshRecent !== false,
            refreshNotifications: pendingRefreshOptions.refreshNotifications !== false || options.refreshNotifications !== false,
            force: !!(pendingRefreshOptions.force || options.force)
        };
    }
    ...
    refreshDebounceTimer = setTimeout(async () => {
        ...
        await executeRefreshApplicationState();
    }, 100);
}

async function executeRefreshApplicationState() {
    if (isRefreshStateInProgress) {
        hasPendingRefreshState = true;
        return;
    }

    const currentVersion = ++stateVersion;
    isRefreshStateInProgress = true;
    const currentOptions = { ...pendingRefreshOptions };
    pendingRefreshOptions = {};

    devTime('refreshApplicationState');
    ...
    try {
        ...
        if (currentOptions.refreshPending !== false) {
            try {
                await updatePendingCount();
            } catch (err) {
                console.error("Pending count update failed:", err);
            }
        }
        if (currentVersion !== stateVersion) return;
        ...
        if (currentOptions.refreshPending !== false) {
            try {
                ...
                console.error("Requests list render failed:", err);
            }
        }
        if (currentVersion !== stateVersion) return;

        if (currentOptions.refreshDirectory !== false || currentOptions.refreshDashboard !== false || currentOptions.refreshRecent !== false) {
            try {
                renderCurrentViewFromState({ force: currentOptions.force });
            } catch (err) {
                console.error("Main view render failed:", err);
            }
        }
        if (currentVersion !== stateVersion) return;

        if (currentOptions.refreshNotifications !== false) {
            try {
        ...
        console.error("Critical error during refreshApplicationState execution:", err);
    } finally {
        devTimeEnd('refreshApplicationState');
        isRefreshStateInProgress = false;

        if (hasPendingRefreshState) {
            hasPendingRefreshState = false;
            await executeRefreshApplicationState();
        }
    }
}
...
async function refreshAppUI() {
    await refreshApplicationState({ persist: false });
}
    ...
    saveEmailBtn.onclick = async function (event) {
        ...
        try {
            ...
            if (wasApprovingRequestId) {
                ...
                // Mutate memory state
                companyEmails = companyEmails.filter(e => !(e.isRequest && (e.requestId === wasApprovingRequestId || e.mail === emailVal)));
                if (pendingRequestsCount > 0) pendingRequestsCount--;
            }
            ...
            // Let the button display "✓ Enregistré"
            if (saveEmailBtn) {
                saveEmailBtn.innerText = '✓ Enregistré';
                saveEmailBtn.classList.add('saved-success');
            }

            // Wait 600ms for user feedback
            ...
            setEmailSaveBusy(false);
            if (saveEmailBtn) {
                saveEmailBtn.classList.remove('saved-success');
            }
        }
    };
}
...
async function handleChangeUpdateDate() {
    ...
    openModal('dateUpdateModal');
}

function closeDateModal() {
    devDebug('[WRAPPER CLOSE] dateUpdateModal');
    closeModal('dateUpdateModal');
}
...
function initSystemDateListener() {
    ...
    systemDateUnsubscribe = db.collection('settings').doc('app').onSnapshot((doc) => {
        if (doc.exists && doc.data().lastUpdate) {
            ...
            devLog("System date synced in real-time:", newDate);
...
// Universal close functions mapping for modals
const modalCloseHandlers = {
    'emailAdminModal': () => { if (typeof closeAdminModal === 'function') closeAdminModal(); },
    'deleteConfirmModal': () => { if (typeof closeDeleteModal === 'function') closeDeleteModal(); },
    'dateUpdateModal': () => { if (typeof closeDateModal === 'function') closeDateModal(); },
    'logoutConfirmModal': () => { if (typeof closeLogoutModal === 'function') closeLogoutModal(); },
    'unitDeleteConfirmModal': () => { if (typeof closeUnitDeleteModal === 'function') closeUnitDeleteModal(); },
    'unitManagementModal': () => { if (typeof closeUnitManagement === 'function') closeUnitManagement(); },
    'serviceSelectionModal': () => { if (typeof closeServiceSelection === 'function') closeServiceSelection(); },
    'employeeDetailsModal': () => { if (typeof closeEmployeeDetailsModal === 'function') closeEmployeeDetailsModal(); },
    'configModal': () => { if (typeof closeConfigModal === 'function') closeConfigModal(); },
    'installModal': () => { if (typeof closeInstallModal === 'function') closeInstallModal(); },
    'desktopModal': () => { if (typeof closeDesktopModal === 'function') closeDesktopModal(); },
    'supportModal': () => { if (typeof closeSupportModal === 'function') closeSupportModal(); },
    'thunderbirdModal': () => { if (typeof closeThunderbirdModal === 'function') closeThunderbirdModal(); },
    'webmailModal': () => { if (typeof closeWebmailModal === 'function') closeWebmailModal(); },
    'correctionModal': () => { if (typeof closeCorrectionModal === 'function') closeCorrectionModal(); },
    'correctionSuccessModal': () => { if (typeof closeCorrectionSuccessModal === 'function') closeCorrectionSuccessModal(); },
    'deactivationModal': () => { if (typeof closeDeactivationModal === 'function') closeDeactivationModal(); },
    'deactivationSuccessModal': () => { if (typeof closeDeactivationSuccessModal === 'function') closeDeactivationSuccessModal(); },
    'correctionInstructionModal': () => { if (typeof closeCorrectionInstructionModal === 'function') closeCorrectionInstructionModal(); },
    'deactivationConfirmModal': () => { if (typeof closeDeactivationConfirmModal === 'function') closeDeactivationConfirmModal(); },
    'requestModal': () => { if (typeof closeRequestModal === 'function') closeRequestModal(); },
    'requestSuccessModal': () => { if (typeof closeRequestSuccessModal === 'function') closeRequestSuccessModal(); },
    'requestsListModal': () => { if (typeof closeRequestsListModal === 'function') closeRequestsListModal(); },
    'requestDetailsModal': () => { if (typeof closeRequestDetailsModal === 'function') closeRequestDetailsModal(); },
    'rejectRequestModal': () => { if (typeof closeRejectRequestModal === 'function') closeRejectRequestModal(); },
    'devModal': () => { if (typeof closeDevModal === 'function') closeDevModal(); },
    'companyModal': () => { if (typeof closeCompanyModal === 'function') closeCompanyModal(); },
    'broadcastModal': () => { if (typeof closeBroadcastModal === 'function') closeBroadcastModal(); },
    'broadcastSuccessModal': () => { if (typeof closeBroadcastSuccessModal === 'function') closeBroadcastSuccessModal(); },
    'signatureSuccessModal': () => { if (typeof closeSignatureSuccessModal === 'function') closeSignatureSuccessModal(); },
    'notifDeleteConfirmModal': () => { if (typeof closeNotifDeleteConfirmModal === 'function') closeNotifDeleteConfirmModal(); },
    'notifDeleteSuccessModal': () => { if (typeof closeNotifDeleteSuccessModal === 'function') closeNotifDeleteSuccessModal(); },
    'profileModal': () => { if (typeof closeProfileModal === 'function') closeProfileModal(); },
    'notifDetailModal': () => { if (typeof closeNotifDetail === 'function') closeNotifDetail(); },
    'cacheSuccessModal': () => { window.location.reload(true); },
    'authModal': () => { if (typeof closeAuthModal === 'function') closeAuthModal(); }
};

const ModalManager = {
    modalRules: {
        'emailAdminModal': ['directory'],
        'deleteConfirmModal': ['directory'],
        'employeeDetailsModal': ['directory'],
        'unitManagementModal': ['stats', 'directory'],
        'unitDeleteConfirmModal': ['stats', 'directory'],
        'supportModal': ['tools', 'contact']
    },

    // Get the currently active page name from DOM state
    getActivePage() {
        const activeSection = document.querySelector('.page-section.active');
        if (!activeSection) return 'dashboard'; // Default fallback
        return activeSection.id.replace(/^page/, '').toLowerCase();
    },

    // Check if the modal is allowed on the current page
    validateSource(modalId) {
        const targetId = modalId === 'editModal' ? 'emailAdminModal' : modalId;
        const allowedPages = this.modalRules[targetId];
        if (!allowedPages) return true; // No rules, allowed anywhere
        const activePage = this.getActivePage();
        return allowedPages.includes(activePage);
    },

    // Reset all target and edit/delete variables when closing modals
    resetModalState(modalId) {
        const targetId = modalId === 'editModal' ? 'emailAdminModal' : modalId;
        devDebug('[ModalManager] Resetting state for:', targetId);

        if (targetId === 'emailAdminModal') {
            originalEmailForEdit = null;
            isEditMode = false;
            isSavingEmail = false;
        ...
        if (targetId === 'deleteConfirmModal') {
            ...
            if (typeof setDeleteBusy === 'function') setDeleteBusy(false);
        }
        if (targetId === 'employeeDetailsModal') {
            currentEmployeeDetails = null;
        }
        if (targetId === 'unitDeleteConfirmModal') {
            if (typeof unitIdToDelete !== 'undefined') unitIdToDelete = null;
        }
        if (targetId === 'notifDeleteConfirmModal') {
            if (typeof notifIdToDelete !== 'undefined') notifIdToDelete = null;
        }
    },

    async open(modalId) {
        const targetId = modalId === 'editModal' ? 'emailAdminModal' : modalId;
        devLog("Opening modal via ModalManager:", targetId);

        // Source Validation
        if (!this.validateSource(targetId)) {
            console.warn(`[ModalManager] Access Denied: Modal '${targetId}' is not allowed on page '${this.getActivePage()}'`);
            return;
        }

        const modal = document.getElementById(targetId);
        if (modal && modal.classList.contains('show')) {
            console.warn("Modal is already open:", targetId);
            return;
        }
        ...
        if (modal) {
            ...
            const modalBox = modal.querySelector('.modal-box, .employee-details-modal, .config-modal');
            if (modalBox) {
                modalBox.style.willChange = 'transform, opacity';
            }

            currentOpenModal = targetId === 'emailAdminModal' ? 'editModal' : targetId;
        }

        await new Promise(resolve => setTimeout(resolve, 220));

        // Clean will-change after transition completes
        if (modal) {
            const modalBox = modal.querySelector('.modal-box, .employee-details-modal, .config-modal');
            if (modalBox) {
                modalBox.style.willChange = '';
    ...
    async close(modalId) {
        const targetId = modalId === 'editModal' ? 'emailAdminModal' : modalId;
        devDebug('[ModalManager] Closing modal:', targetId);

        const modal = document.getElementById(targetId);
        if (modal) {
            if (modal.classList.contains('show')) {
                lastModalCloseTime = Date.now();
                shouldIgnoreNextMousedown = true;
                shouldIgnoreNextMouseup = true;
                shouldIgnoreNextClick = true;
            }

            // Set will-change for transition
            const modalBox = modal.querySelector('.modal-box, .employee-details-modal, .config-modal');
            if (modalBox) {
                modalBox.style.willChange = 'transform, opacity';
            }
            ...
            // Wait for transitions to finish, then clear will-change
            setTimeout(() => {
                if (modalBox) modalBox.style.willChange = '';
            }, 250);
        }

        const closedAlias = targetId === 'emailAdminModal' ? 'editModal' : targetId;
        if (currentOpenModal === closedAlias || currentOpenModal === targetId) {
            currentOpenModal = null;
        }

        this.resetModalState(targetId);

        await new Promise(resolve => setTimeout(resolve, 220));
    },

    closeAll() {
        devDebug('[ModalManager] Closing all modals systematically');
        let closedAny = false;

        // Use high-level handlers to ensure proper reset hooks are run
        ...
        // Fallback for overlays or other popups that are not in close handlers
        const allModals = document.querySelectorAll('.modal, .modal-overlay, .config-modal-overlay');
        allModals.forEach(modal => {
            if (modal.classList.contains('show')) {
                ...
                // Reset state dynamically based on ID
                if (modal.id) {
...
// Open a modal ensuring no other modal is open
function openModal(modalId) {
    measureTransition('Open Modal: ' + modalId, async () => {
        await ModalManager.open(modalId);
    });
}

// Close a specific modal (Low-Level Function: Modifies the DOM only, no handler callbacks to avoid loops)
function closeModal(modalId) {
    measureTransition('Close Modal: ' + modalId, async () => {
        await ModalManager.close(modalId);
    });
}

// Close all modals systematically
function closeAllModals() {
    ModalManager.closeAll();
}

// Outside clicks to close any open modal
window.addEventListener('click', (e) => {
    ...
    if (isModal) {
        ...
        if (modalId) {
            // Trigger high-level handler for proper cleanup (resets etc.), which in turn calls low-level closeModal
...
function showBrowserNotification(title, body) {
    if (Notification.permission === 'granted') {
        ...
        if (isSupportedOrigin && 'serviceWorker' in navigator) {
            try {
                    ...
                    try {
                        navigator.serviceWorker.ready.then(registration => {
                            try {
                                ...
                                console.warn("SW showNotification failed:", showErr);
                            }
                        }).catch(err => {
                            ...
                            try {
                                ...
                                console.warn("Fallback Notification failed:", notiErr);
                            }
                        });
                        shownViaSw = true;
                    } catch (readyErr) {
                        console.warn("SW ready property access failed:", readyErr);
                    }
                }
            } catch (swErr) {
                console.warn("SW notification access failed:", swErr);
            }
        }
...
function deleteAnnouncement(id) {
    notifIdToDelete = id;
    openModal('notifDeleteConfirmModal');
}

function closeNotifDeleteConfirmModal() {
    devDebug('[WRAPPER CLOSE] notifDeleteConfirmModal');
    closeModal('notifDeleteConfirmModal');
    notifIdToDelete = null;
}

function closeNotifDeleteSuccessModal() {
    devDebug('[WRAPPER CLOSE] notifDeleteSuccessModal');
    closeModal('notifDeleteSuccessModal');
}
    ...
    if (confirmBtn) {
        confirmBtn.onclick = async () => {
            ...
            try {
                ...
                // Show success modal
                openModal('notifDeleteSuccessModal');
            } catch (error) {
...
function showNotifDetail(id) {
    ...
    if (modal && title && date && body) {
        ...
        // Helper to detect code content
        const isCodeContent = (text) => {
            if (!text) return false;
            const trimmed = text.trim();
            if (trimmed.startsWith('<!--') || trimmed.startsWith('<?xml') || (trimmed.startsWith('<') && trimmed.endsWith('>'))) {
                return true;
            }
        ...
        if (isCodeContent(n.message)) {
            body.style.setProperty('font-family', "'Fira Code', 'Consolas', 'Courier New', monospace", 'important');
            body.style.setProperty('background', '#0f172a', 'important');
            body.style.setProperty('background-image', 'none', 'important');
            body.style.setProperty('color', '#38bdf8', 'important');
            body.style.setProperty('border-color', '#1e293b', 'important');
            body.style.setProperty('padding', '15px 20px 20px', 'important');
            body.style.setProperty('box-shadow', 'none', 'important');

            const escaped = n.message
                .replace(/&/g, "&amp;")
                .replace(/</g, "&lt;")
                .replace(/>/g, "&gt;")
                .replace(/"/g, "&quot;")
            ...
            body.innerHTML = `
<div class="code-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid #1e293b; font-family: system-ui, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 0.72rem; color: #64748b; font-weight: 700; letter-spacing: 0.5px; text-transform: uppercase; user-select: none;">
    <span><i class="fas fa-code" style="color: #38bdf8; margin-right: 6px;"></i>CODE SOURCE</span>
    <button onclick="window.copyCodeFromNotif(event)" style="background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.12); color: #cbd5e1; padding: 4px 10px; border-radius: 6px; cursor: pointer; display: flex; align-items: center; gap: 6px; font-size: 0.72rem; font-weight: 600; transition: all 0.2s;"><i class="far fa-copy"></i><span>Copier</span></button>
</div>
<pre style="margin: 0; padding: 0; font-family: inherit; font-size: 0.84rem; color: #e2e8f0; line-height: 1.6; overflow-x: auto; white-space: pre-wrap; word-break: break-all; text-align: left; direction: ltr;">${escaped}</pre>`;

            window.copyCodeFromNotif = function (event) {
                event.stopPropagation();
                navigator.clipboard.writeText(n.message).then(() => {
                    ...
                    showCopyNotification('Code copié avec succès !');
                });
            };
        } else {
        ...
        const configs = {
            info: { color: '#3b82f6', bg: 'linear-gradient(165deg, #1e3a8a 0%, #3b82f6 100%)', label: 'Information', icon: 'fa-envelope-open-text' },
            warning: { color: '#f59e0b', bg: 'linear-gradient(165deg, #92400e 0%, #f59e0b 100%)', label: 'Attention', icon: 'fa-circle-exclamation' },
            critical: { color: '#ef4444', bg: 'linear-gradient(165deg, #7f1d1d 0%, #ef4444 100%)', label: 'Message Critique', icon: 'fa-triangle-exclamation' }
        };
    ...
    if (broadcastForm) {
        broadcastForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                // Show professional success modal
                openModal('broadcastSuccessModal');
            } catch (error) {
                console.error(error);
...
function closeBroadcastSuccessModal() {
    closeModal('broadcastSuccessModal');
}
...
function closeCorrectionModal() {
    devDebug('[WRAPPER CLOSE] correctionModal');
...
function openDeactivationModal() {
    openModal('deactivationModal');
    if (typeof populateSystemDatalists === 'function') populateSystemDatalists();
}

function closeDeactivationModal() {
    devDebug('[WRAPPER CLOSE] deactivationModal');
...
function openCorrectionSuccessModal() {
    openModal('correctionSuccessModal');
}

function closeCorrectionSuccessModal() {
    devDebug('[WRAPPER CLOSE] correctionSuccessModal');
    closeModal('correctionSuccessModal');
}

function openDeactivationSuccessModal() {
    openModal('deactivationSuccessModal');
}

function closeDeactivationSuccessModal() {
    devDebug('[WRAPPER CLOSE] deactivationSuccessModal');
    closeModal('deactivationSuccessModal');
}

function openCorrectionInstructionModal() {
    openModal('correctionInstructionModal');
}

function closeCorrectionInstructionModal() {
    devDebug('[WRAPPER CLOSE] correctionInstructionModal');
    closeModal('correctionInstructionModal');
...
function openDeactivationConfirmModal(email, requestId) {
    ...
    openModal('deactivationConfirmModal');
}

function closeDeactivationConfirmModal() {
    devDebug('[WRAPPER CLOSE] deactivationConfirmModal');
    closeModal('deactivationConfirmModal');
}
...
async function executeAccountBlocking(email, requestId) {
    try {
        ...
        if (!emailSnapshot.empty) {
            ...
            const targetEmail = companyEmails.find(e => e.mail === email);
            if (targetEmail) {
                targetEmail.status = 'Désactivée';
                targetEmail.updatedAt = new Date().toISOString();
            }
            companyEmails = companyEmails.filter(e => !(e.isRequest && (e.requestId === requestId || e.mail === email)));
            if (pendingRequestsCount > 0) pendingRequestsCount--;

            showCopyNotification('✅ Compte désactivé avec succès');
...
function closeRequestModal() {
    devDebug('[WRAPPER CLOSE] requestModal');
...
function closeRequestSuccessModal() {
    devDebug('[WRAPPER CLOSE] requestSuccessModal');
    closeModal('requestSuccessModal');
}
    ...
    if (requestForm) {
        requestForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await refreshApplicationState({ refreshPending: true, persist: false });
            } catch (error) {
...
async function openRequestsListModal() {
    openModal('requestsListModal');
    window.isRequestsExpanded = false; // Reset expansion state
    await refreshApplicationState({ refreshPending: true, persist: false });
}

async function expandRequests() {
    window.isRequestsExpanded = true;
    await refreshApplicationState({ refreshPending: true, persist: false });
}

function closeRequestsListModal() {
    devDebug('[WRAPPER CLOSE] requestsListModal');
    closeModal('requestsListModal');
}
    ...
    const skeletonTimer = setTimeout(() => {
        if (!isDataLoaded) {
            container.innerHTML = Array(3).fill(0).map(() => `
                <div class="req-list-item skeleton-item" style="display: flex; align-items: center; gap: 15px; padding: 15px; border-bottom: 1px solid #f1f5f9;">
                    <div class="skeleton-icon skeleton-shimmer" style="width: 40px; height: 40px; border-radius: 10px; flex-shrink: 0;"></div>
                    <div style="flex: 1; display: flex; flex-direction: column; gap: 6px;">
                        <div class="skeleton-text skeleton-shimmer" style="width: 50%; height: 14px; border-radius: 4px;"></div>
                        <div class="skeleton-text skeleton-shimmer" style="width: 70%; height: 10px; border-radius: 4px;"></div>
                        <div class="skeleton-text skeleton-shimmer" style="width: 40%; height: 10px; border-radius: 4px;"></div>
                    </div>
                    <div class="skeleton-button skeleton-shimmer" style="width: 50px; height: 26px; border-radius: 8px; flex-shrink: 0;"></div>
                </div>
            `).join('');
        }
    }, 150);
...
async function showRequestDetails(requestId) {
    try {
        devDebug("Loading details for request:", requestId);
        ...
        window.currentViewingRequestData = data;

        const type = (data.type || 'new_account').trim().toLowerCase();
        devDebug("Request Type detected:", type);
...
function closeRequestDetailsModal() {
    devDebug('[WRAPPER CLOSE] requestDetailsModal');
...
async function approveRequest(requestId) {
    try {
        if (requestId.startsWith('email-')) {
            ...
            // Mutate state in memory
            const targetEmail = companyEmails.find(e => e.mail === emailObj.mail);
            if (targetEmail) {
                targetEmail.status = 'Active';
                targetEmail.updatedAt = new Date().toISOString();
            }
        ...
        // Get request data from window cache or fetch it
        let data = null;
        if (window.currentViewingRequestId === requestId && window.currentViewingRequestData) {
            data = window.currentViewingRequestData;
        } else {
            const doc = await db.collection('pendingRequests').doc(requestId).get();
            if (!doc.exists) return;
            data = doc.data();
        ...
        if (data.type === 'correction') {
            ...
            // Mutate state in memory
            companyEmails = companyEmails.filter(e => !(e.isRequest && e.requestId === requestId));
            if (pendingRequestsCount > 0) pendingRequestsCount--;
        ...
        // Automatic direct approval and saving
        showCopyNotification('⏳ Approbation du compte...');
        ...
        // Create/Update the email document in emails collection
        await withOperationTimeout(db.collection('emails').doc(newDocId).set(emailData, { merge: true }), 'Email save', 10000);

        // Update request status to approved
        await withOperationTimeout(db.collection('pendingRequests').doc(requestId).update({
            status: 'approved',
            resolvedAt: firebase.firestore.FieldValue.serverTimestamp()
        }), 'Request approval', 10000);

        showCopyNotification('✅ Demande approuvée et compte créé avec succès !');
        addSystemLog(`Approbation et création du compte : ${emailVal}`);

        // Mutate memory state
        companyEmails = companyEmails.filter(e => !(e.isRequest && (e.requestId === requestId || e.mail === emailVal)));
        if (pendingRequestsCount > 0) pendingRequestsCount--;
        ...
        showCopyNotification('❌ Erreur : ' + (error.message || 'approbation impossible'));
    }
}
...
function closeRejectRequestModal() {
    devDebug('[WRAPPER CLOSE] rejectRequestModal');
...
// Dynamic will-change manager for modals to optimize transition performance v6.1.0
function initModalPerformanceObserver() {
        ...
        mutations.forEach((mutation) => {
            if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                ...
                const isModal = target.classList.contains('modal') ||
                    ...
                    target.classList.contains('config-modal-overlay') ||
                    target.classList.contains('employee-details-overlay');

                if (isModal) {
                    const modalBox = target.querySelector('.modal-content, .modal-box, .employee-details-modal, .success-modal-content, .request-modal-content') || target;
                    modalBox.style.willChange = 'transform, opacity';

                    // Clear after transition duration
                    setTimeout(() => {
                        modalBox.style.willChange = '';
                    }, 300);
                }
    ...
    // Observe changes to all elements with class modal or modal-overlay
    document.querySelectorAll('.modal, .modal-overlay, .config-modal-overlay, .employee-details-overlay').forEach(modal => {
