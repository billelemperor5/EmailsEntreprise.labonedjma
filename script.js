...
function populateEmailDatalist() {
    const datalist = document.getElementById('systemEmails');
    if (!datalist) return;
    
    // Use companyEmails (existing active emails)
    const activeEmails = companyEmails
        .filter(e => !e.isRequest && e.status === 'Active')
        .map(e => e.mail);
        
    datalist.innerHTML = '';
    [...new Set(activeEmails)].forEach(mail => {
        const opt = document.createElement('option');
        opt.value = mail;
